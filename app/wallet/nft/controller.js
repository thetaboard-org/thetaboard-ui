    ...
    try {
      if (ethereum && ethereum.isConnected()) {
        if (parseInt(ethereum.chainId) !== 361) {
          ...
          this.utils.successNotify(this.intl.t('notif.success_nft'));
        } else {
          this.utils.errorNotify(this.intl.t('notif.not_theta_blockchain'));
        }
      } else {
        this.utils.errorNotify(this.intl.t('notif.no_metamask'));
      }
    } catch (e) {
      this.utils.errorNotify(e.message);
    }

    /* if it is not a theta blockchain*/
